...
export const HOSP_STORAGE_KEY   = "acf_hospitalization_records";
export const HOSP_CHANNEL        = "acf_hospitalization_channel";
export const HOSP_EVENT          = "acf_hospitalization_updated";
...
export function loadHospRecords(): HospRecord[] {
  try {
    const raw = localStorage.getItem(HOSP_STORAGE_KEY);
...
export function saveHospRecord(rec: HospRecord): void {
  try {
    ...
    localStorage.setItem(HOSP_STORAGE_KEY, JSON.stringify(all));
  } catch {}
}
...
export function advanceSurgeryStage(
  ...
  all[idx] = rec;
  try { localStorage.setItem(HOSP_STORAGE_KEY, JSON.stringify(all)); } catch {}
...
function dispatchHospNotification(rec: HospRecord, stage: SurgeryStage): void {
  ...
  try {
    const ch = new BroadcastChannel(HOSP_CHANNEL);
    ch.postMessage({ type: HOSP_EVENT, recordId: rec.id, stage });
    ch.close();
  } catch {}
}
...
export function broadcastHospUpdate(): void {
  window.dispatchEvent(new CustomEvent(HOSP_EVENT));
  try {
    const ch = new BroadcastChannel(HOSP_CHANNEL);
    ch.postMessage({ type: HOSP_EVENT });
    ch.close();
  } catch {}
}
...
export function subscribeToHospitalization(cb: () => void): () => void {
  const onEvent = () => cb();
  window.addEventListener(HOSP_EVENT, onEvent);
  let ch: BroadcastChannel | null = null;
  try {
    ch = new BroadcastChannel(HOSP_CHANNEL);
    ch.onmessage = () => cb();
  } catch {}
  return () => {
    window.removeEventListener(HOSP_EVENT, onEvent);
    ch?.close();
  };
}
